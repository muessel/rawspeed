...
void FujiDecompressor::fuji_fill_buffer (struct fuji_compressed_block *info)
{
  if (info->cur_pos >= info->cur_buf_size) {
...
#endif
    {
      ...
      //info->cur_buf_size =info->cur_buf_size = std::min (info->max_read_size, FUJI_BUF_SIZE);
	...
	/*
	memcpy(info->cur_buf,
	       input.getData(info->cur_buf_offset, info->cur_buf_size),
	       info->cur_buf_size);
	*/
      }
...
void FujiDecompressor::init_fuji_block (struct fuji_compressed_block* info, const struct fuji_compressed_params *params, uint64 raw_offset, unsigned dsize)
{
  info->linealloc = (ushort*)calloc (sizeof (ushort), _ltotal * (params->line_width + 2));
  //merror (info->linealloc, "init_fuji_block()");
  //info->input = ifp;
  //uint64 fsize = info->input->size;
  // TODO check?
  ...
  // init buffer
  //info->cur_buf = (uchar8*)malloc (FUJI_BUF_SIZE);
...
void FujiDecompressor::copy_line_to_xtrans (struct fuji_compressed_block* info, int cur_line, int cur_block, int cur_block_width)
{
  ushort *lineBufB[3];
  ushort *lineBufG[6];
  ushort *lineBufR[3];
  ...
  //ushort* raw_block_data = (ushort*)  mImg.get()->getData() + offset;
  // TODO check
...
void FujiDecompressor::fuji_zerobits (struct fuji_compressed_block* info, int *count)
{
  uchar8 zero = 0;
  *count = 0;
...
void FujiDecompressor::fuji_read_code (struct fuji_compressed_block* info, int *data, int bits_to_read)
{
...
int FujiDecompressor::fuji_decode_sample_even (struct fuji_compressed_block* info, const struct fuji_compressed_params * params, ushort* line_buf, int pos, struct int_pair* grads)
{
  int interp_val = 0;
  int errcnt = 0;
...
int FujiDecompressor::fuji_decode_sample_odd (struct fuji_compressed_block* info, const struct fuji_compressed_params * params, ushort* line_buf, int pos, struct int_pair* grads)
{
  int interp_val = 0;
  int errcnt = 0;
...
void FujiDecompressor::fuji_decode_interpolation_even (int line_width, ushort* line_buf, int pos)
{
  ushort* line_buf_cur = line_buf + pos;
...
void FujiDecompressor::fuji_extend_generic (ushort *linebuf[_ltotal], int line_width, int start, int end)
{
...
void FujiDecompressor::fuji_extend_red (ushort *linebuf[_ltotal], int line_width)
{
  fuji_extend_generic (linebuf, line_width, _R2, _R4);
}

void FujiDecompressor::fuji_extend_green (ushort *linebuf[_ltotal], int line_width)
{
  fuji_extend_generic (linebuf, line_width, _G2, _G7);
}

void FujiDecompressor::fuji_extend_blue (ushort *linebuf[_ltotal], int line_width)
{
  fuji_extend_generic (linebuf, line_width, _B2, _B4);
}

void FujiDecompressor::xtrans_decode_block (struct fuji_compressed_block* info, const struct fuji_compressed_params *params, int cur_line)
{
...
void FujiDecompressor::fuji_compressed_load_raw()
{
  ...
  memcpy(block_sizes, input.getData(data_offset, sizeof (unsigned)*fuji_total_blocks), sizeof (unsigned)*fuji_total_blocks);
...
void FujiDecompressor::fuji_decode_loop (const struct fuji_compressed_params* common_info, int count, uint64* raw_block_offsets, unsigned *block_sizes)
{
...
void FujiDecompressor::parse_fuji_compressed_header()
{
  ...
  const uchar8 *header = input.getData(data_offset, 16);
